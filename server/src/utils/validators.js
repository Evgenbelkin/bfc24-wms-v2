 ...
 *
 * 2) Остальные товарные группы (переменная длина серийного номера) — между
 *    серийным номером и AI(91) обязан быть один служебный байт-разделитель
 *    GS (0x1D, невидимый) — без него не понять, где заканчивается серийный
 *    номер переменной длины. А вот между AI(91) (фиксированные 4 символа) и
 *    AI(92) разделитель СТРОГО не обязателен (длина уже известна заранее),
 *    но некоторые кодировщики/принтеры его всё равно ставят — оба варианта
 *    (с этим вторым GS и без него) считаем валидными.
 *    Реальный пример на живом ТСД-скане (сериал 13 симв., один GS перед
 *    AI91, ещё один GS перед AI92) подтвердил: второй GS — это нормальный
 *    вариант кодирования, а не поломка. Первая версия этой проверки требовала
 *    РОВНО один GS всегда и ошибочно резала такие корректные коды.
 *
 * В обоих случаях ловим реально встретившуюся поломку: отсутствие ПЕРВОГО,
 * обязательного разделителя после переменного серийного номера (тогда
 * невозможно понять, где заканчивается серийный номер), либо не совпадающий
 * по длине "хвост" AI91/AI92 (испорченные данные). Сканирование камерой
 * телефона (BarcodeDetector в браузере) ненадёжно передаёт байт-разделитель —
 * то теряет его вовсе, то смещает — из-за чего код перестаёт совпадать ни с
 * одним из ожидаемых видов структуры. Обычный физический 2D-сканер/ТСД в
 * правильном режиме передаёт байты стабильно. Функция не пытается разобрать
...
function hasValidKizStructure(value) {
  ...
  const rest = s.slice(18); // после "01"+14 цифр GTIN+"21"

  // Лёгкая промышленность: все поля фиксированной длины, разделители не нужны совсем.
  if (/^.{13}91.{4}92.{44}$/.test(rest) && !rest.includes('\x1d')) return true;

  // Остальные категории: серийный номер переменной длины — первый GS обязателен.
  const gsIdx = rest.indexOf('\x1d');
  if (gsIdx === -1) return false; // разделитель потерян вовсе — без него не разобрать код
  const serial = rest.slice(0, gsIdx);
  if (!serial) return false;
  // Второй (необязательный) GS перед AI92 — если есть, просто убираем перед проверкой хвоста.
  const tail = rest.slice(gsIdx + 1).replace(/\x1d/g, '');
  return /^91.{4}92.{44}$/.test(tail);
}

/**
 * Нет ли в коде "Честный знак" символов вне печатной ASCII-таблицы (кроме
 * служебных разделителей GS1) — реальный инцидент (08-09.09.2026) показал:
 * при сканировании DataMatrix, если раскладка клавиатуры на терминале/ТСД
 * в момент скана оказалась русской (сканер обычно эмулирует HID-клавиатуру
 * и печатает "как напечатал бы человек" на ТЕКУЩЕЙ раскладке ОС), часть
 * латинских букв кода незаметно печатается похожими кириллическими
 * буквами-омоглифами (например латинская 'C'/'H'/'A' → кириллическая
 * 'С'/'Н'/'А' — визуально неотличимо). Длина и структура (см.
 * hasValidKizStructure — она проверяет только ПОЗИЦИИ разделителей, а не
 * сами символы) при этом остаются полностью корректными, код спокойно
 * проходит импорт/приёмку и "используется", а проваливается только через
 * дни, при реальной отправке в WB — с ошибкой "sgtinHasNonLatinSymbols",
 * когда поставка уже физически может стоять в пути. Ловим здесь, на входе.
 */
function hasOnlyAsciiChars(value) {
  const s = String(value || '');
  for (let i = 0; i < s.length; i++) {
    const code = s.charCodeAt(i);
    if (code === 0x1d || code === 0x04 || code === 0x1c) continue; // служебные разделители GS1/FNC1 — норма
