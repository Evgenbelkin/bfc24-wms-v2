...
async function reprintCode({ tenantId, code, userId, employeeId = null, dbClient = null }) {
  const run = async (client) => {
    ...
    // svg возвращаем фронту вместе с заданием печати — реальный инцидент
    // (08-09.09.2026) показал: если принтер-агент на рабочем месте не
    // настроен/не запущен, задание тихо остаётся в очереди 'new' навсегда,
    // а у оператора нет вообще никакой возможности увидеть/распечатать код
    // вручную (в отличие от стикера ВБ или QR поставки, у которых уже есть
    // кнопка "Открыть"). Фронт открывает этот SVG в новой вкладке как
    // аварийный фолбэк, независимо от того, доедет ли задание до принтера.
    return { jobId: pjRes.rows[0].id, code: row.code, svg };
  };
