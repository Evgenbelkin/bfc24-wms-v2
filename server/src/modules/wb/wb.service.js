...
async function syncOrdersForAccount({ tenantId, accountId, apiToken, clientId = null }) {
  // clientId больше не используется в этой функции (раньше нужен был для
  // triggerRedistributionForClient по touchedBarcodes, см. комментарий ниже) -
  // оставлен в сигнатуре, чтобы не трогать вызывающий код (syncAllAccountsForTenant).
  const { fetched, saved, marked_external } =
    await fetchAndUpsertOrders({ tenantId, accountId, apiToken });

  // ПРАВКА 28.08.2026 (обсуждение архитектуры): раньше здесь стоял пересчёт
  // и пуш остатка в WB сразу по каждому товару, затронутому свежими
  // заказами. Убрано намеренно - сам WB уже уменьшает показываемый остаток
  // на своей стороне в момент создания заказа, без нашего участия (см.
  // комментарий у distributeStockForAccount: "WB уже резервирует остаток на
  // своей стороне в момент создания заказа"). Пушить с нашей стороны ЕЩЁ РАЗ
  // просто по факту нового заказа - лишняя работа: WB API - это SET
  // абсолютного числа, а не "вычти 1", и мы своим пушем НЕ УСКОРЯЕМ уменьшение
  // остатка, WB и так уже его уменьшил сам, раньше, чем мы вообще узнали о
  // заказе.
  //
  // Оригинальный инцидент 2006784214907 (17.08.2026), из-за которого этот
  // триггер когда-то завели, на самом деле лечится ДРУГИМ механизмом, который
  // остаётся без изменений: перед КАЖДЫМ реальным пушем (приёмка/
  // инвентаризация/смена настроек/контрольный пересчёт раз в 8ч,
  // см. distributeStockForAccount) система всегда сначала подтягивает
  // свежайший список заказов и вычитает ещё не отгруженные - именно это
  // не даёт пушу затереть то, что WB уже продал, независимо от того, когда
  // именно этот пуш произошёл. Реактивный пуш "по каждому новому заказу"
  // не добавлял защиты сверх этого - только лишнюю нагрузку на WB API и БД
  // при высоком темпе заказов (проблема особенно заметна у клиентов с
  // ~20 000 заказов/сутки).
