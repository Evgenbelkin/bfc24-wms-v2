...
(function (window) {
  ...
  function onScan(inputSelector, callback) {
    ...
    // Костыль под сканер Mertech CL-2310 P2D (Bluetooth) на приёмке/упаковке.
    // В одной конфигурации этот экземпляр слал завершающий Enter буквальным
    // текстом "[CR]"/"[CR][LF]" вместо настоящей клавиши — обычный keydown с
    // Enter не происходил вовсе. Подхватываем это через input: если значение
    // оканчивается на такой маркер — считаем скан завершённым и отрезаем
    // маркер из данных.
    var idleSubmitTimer = null;
    input.addEventListener('input', function () {
      ...
      // Второй костыль: после того как убрали суффикс на сканере (чтобы
      // избавиться от "[CR]" в тексте выше), сканер вообще перестал слать
      // ЛЮБОЙ завершающий символ — ни настоящий Enter, ни текст. Тогда ни
      // keydown, ни маркер выше не сработают никогда, и скан "зависает" в
      // поле. Подстраховка: если после очередного символа поле не менялось
      // 200мс (пачка символов от сканера идёт за миллисекунды, у человека
      // ручной ввод так быстро не набирается) — считаем скан завершённым
