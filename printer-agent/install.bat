...
rem --- 0. Снимаем блокировку "файл скачан из интернета" со всех файлов в папке.
rem     Без этого Windows будет каждый раз (в т.ч. при каждой перезагрузке,
rem     когда агент запускается сам через автозапуск) показывать окно
rem     "Не удаётся проверить издателя" - и тихий фоновый автозапуск будет
rem     молча не срабатывать, пока кто-то вручную не нажмёт "Запустить".
