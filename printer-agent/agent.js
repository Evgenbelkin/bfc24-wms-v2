...
async function processJob(job) {
  ...
  try {
    ...
    // ВАЖНО: содержимое SVG (и у WB, и у нашего QR) устроено под лист
    // 58х40 landscape - но по факту печатающая головка физически печатает
    // только ~40мм в ширину, а не 58. При странице шириной 58мм принтер
    // вписывал контент в свои реальные 40мм и оставлял пустые поля по
    // бокам, вместо того чтобы использовать всю ширину рулона. Меняем
    // местами: страница 40мм (ширина, под головку) х 58мм (длина, вдоль
    // ленты), содержимое поворачиваем на 90°, чтобы легло без пустот.
