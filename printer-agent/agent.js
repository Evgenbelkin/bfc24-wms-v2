...
async function processJob(job) {
  ...
  try {
    ...
    // Проверено на реальных SVG от WB (см. debug/) - контент уже правильно
    // ориентирован под лист 58х40 landscape, поворот на 90° (пробовали, в т.ч.
    // с обменом местами ширины/длины страницы) НИКАК не повлиял на результат -
    // значит пустые поля по бокам это не наша страница, а собственный
    // шаблон/поля драйвера принтера для этой Stock-заготовки "58x40".
    // Возвращаем без поворота, дальше разбираемся через сам драйвер (кнопка
    // "Edit" у Stock в настройках печати), а не через код агента.
